...
def plot_evoked_joint(evoked, times="peaks", title='', picks=None,
                      exclude=None, show=True, ts_args=None, topomap_args=None,
                      figsize=None):
    ...
    if len(ch_types) > 1:
        ...
        for t in ch_types:  # pick only the corresponding channel type
            ...
            figs.append(
                plot_evoked_joint(ev_, times=times, title=title, show=show,
    ...
    # map_ax = divider.append_axes('top', size='50%', pad=0.05)

    map_ax = [plt.subplot(4, ts, x + 2 + ts) for x in range(ts - 2)]
            #  [(4, ts, x + 2 + ts) for x in range(ts - 2)]
